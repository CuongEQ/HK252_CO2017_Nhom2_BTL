...
// Find the process by PID in `running_list`, `ready_queue` and `mlq_ready_queue` (if supported)
static struct pcb_t *find_proc_by_pid(struct krnl_t *krnl, uint32_t pid)
{
    ...
    proc = find_proc_in_queue(krnl->running_list, pid); // Check in running_list first
    if (proc != NULL)
        return proc;

    proc = find_proc_in_queue(krnl->ready_queue, pid); // Check in ready_queue next
    if (proc != NULL)
        return proc;
...
{
    BYTE value; // Temporary variable to hold the value read from memory for SYSMEM_IO_READ
    ...
    struct pcb_t *caller = find_proc_by_pid(krnl, pid); // Find the caller process by PID
